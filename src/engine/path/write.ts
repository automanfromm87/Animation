...
import { revealPartial } from './pace';
 ...
 * fallbackWidth 是层没给 outlineWidth 时的轮廓线宽;pace 是描轮廓的笔速(null 为按弧长匀速),
 * 只改这一片内部的快慢,片与片的错峰(staggered)不受影响。
 */
export function writeStep(
  layer: PathLayer,
  u: number,
  fallbackWidth: number,
  pace: RevealPace | null = null,
): PathLayer | null {
  if (!(u > 0)) {
    return null;
  }
